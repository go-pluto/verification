...
datatype ('id, 'a) operation = Add "'id" "'a" | Rem "'id set" "'a" | Store "'a" "'id" "'id"

type_synonym ('id, 'a) state = "'a \<rightharpoonup> 'id set"

definition op_elem :: "('id, 'a) operation \<Rightarrow> 'a" where
  "op_elem oper \<equiv> case oper of Add i e \<Rightarrow> e | Rem is e \<Rightarrow> e | Store e mo mn \<Rightarrow> e"
...
definition interpret_op :: "('id, 'a) operation \<Rightarrow> ('id, 'a) state \<rightharpoonup> ('id, 'a) state" ("\<langle>_\<rangle>" [0] 1000) where
  "interpret_op oper state \<equiv>
     let mailbox  = case oper of 
		   Add i e \<Rightarrow> (case state e of None \<Rightarrow> Some {i} | Some f \<Rightarrow> Some (insert i f)) | 
       Rem is e \<Rightarrow> (case state e of None \<Rightarrow> None | Some f \<Rightarrow> (case f - is = {} of True \<Rightarrow> None | False \<Rightarrow> Some (f - is))) |
       Store e mo mn \<Rightarrow> (case state e of None \<Rightarrow> Some {mn} | Some f \<Rightarrow> Some (insert mn (f - {mo})))
     in  Some (state ((op_elem oper) := mailbox))"
...
definition valid_behaviours :: "('id, 'a) state \<Rightarrow> 'id \<times> ('id, 'a) operation \<Rightarrow> bool" where
  "valid_behaviours state msg \<equiv>
     case msg of
       ...
       (i, Rem is e) \<Rightarrow> (case state e of None \<Rightarrow> False | Some f \<Rightarrow> is = f) |
       (i, Store e mo mn) \<Rightarrow> i = mn"

locale orset = network_with_constrained_ops _ interpret_op "\<lambda>x. None" valid_behaviours

lemma add_add_commute:
  shows "(\<langle>Add i1 e1\<rangle> \<rhd> \<langle>Add i2 e2\<rangle>) s = (\<langle>Add i2 e2\<rangle> \<rhd> \<langle>Add i1 e1\<rangle>) s"
  	unfolding interpret_op_def op_elem_def kleisli_def  apply (case_tac "e1=e2") apply simp apply auto
  	by (simp add: insert_commute option.case_eq_if)
...
lemma add_rem_commute:
  ...
  using assms unfolding interpret_op_def kleisli_def op_elem_def apply (case_tac "e1=e2") apply simp apply auto
  	apply (simp add:  insert_Diff_if option.case_eq_if option.discI )
  by (smt option.discI Diff_eq_empty_iff option.sel)
...
lemma rem_rem_commute:
  ...
  unfolding interpret_op_def op_elem_def kleisli_def apply (case_tac "e1=e2") apply simp apply auto
  apply	(case_tac "s e2 = None") proof(auto)
  fix y
  show "e1 = e2 \<Longrightarrow>
         s e2 = Some y \<Longrightarrow>
         s(e2 := case case y \<subseteq> i1 of True \<Rightarrow> None | False \<Rightarrow> Some (y - i1) of None \<Rightarrow> None
                 | Some f \<Rightarrow> case f - i2 = {} of True \<Rightarrow> None | False \<Rightarrow> Some (f - i2)) =
         s(e2 := case case y \<subseteq> i2 of True \<Rightarrow> None | False \<Rightarrow> Some (y - i2) of None \<Rightarrow> None
                 | Some f \<Rightarrow> case f - i1 = {} of True \<Rightarrow> None | False \<Rightarrow> Some (f - i1))"	
    apply (case_tac "y \<subseteq> i1") apply auto 
     apply (smt Diff_eq_empty_iff Diff_subset option.case_eq_if option.sel subset_trans) sorry
      qed
      (*by (smt Diff_Int2 Diff_Int_distrib2 Diff_eq Diff_eq_empty_iff Diff_subset option.simps(4) option.simps(5)) *)
  	

lemma rem_store_commute:
	assumes "mn \<notin> is"
  shows "(\<langle>Store e1 mo mn\<rangle> \<rhd> \<langle>Rem is e2\<rangle>) s = (\<langle>Rem is e2\<rangle> \<rhd> \<langle>Store e1 mo mn\<rangle>) s"
  unfolding interpret_op_def kleisli_def op_elem_def apply (case_tac "e1=e2") apply simp apply auto
  apply	(case_tac "s e2 = None") apply auto apply (simp add: Diff_triv assms)
  using assms 
  by (smt Diff_eq_empty_iff Diff_insert Diff_insert0 Diff_insert2 insert_Diff insert_Diff_if option.case_eq_if option.discI option.sel)
  	
lemma add_store_commute:
	assumes "mo \<noteq> i"
  shows "(\<langle>Store e1 mo mn\<rangle> \<rhd> \<langle>Add i e2\<rangle>) s = (\<langle>Add i e2\<rangle> \<rhd> \<langle>Store e1 mo mn\<rangle>) s"
  unfolding interpret_op_def kleisli_def op_elem_def apply (case_tac "e1=e2") apply simp apply auto	using assms
  apply	(case_tac "s e2 = None") by auto
  	
lemma store_store_commute:
	assumes "mn1 \<noteq> mo2" and "mn2 \<noteq> mo1"
  shows "(\<langle>Store e1 mo1 mn1\<rangle> \<rhd> \<langle>Store e2 mo2 mn2\<rangle>) s = (\<langle>Store e2 mo2 mn2\<rangle> \<rhd> \<langle>Store e1 mo1 mn1\<rangle>) s"
  unfolding interpret_op_def kleisli_def op_elem_def apply (case_tac "e1=e2") apply simp apply auto	apply (case_tac "s e2 = None") apply auto 
