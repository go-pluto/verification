...
lemma (in orset) Deliver_added_files:
  ...
  shows "Deliver (i, Append i e) \<in> set xs"
...
lemma (in orset) Broadcast_Expunge_Deliver_prefix_closed:
  ...
  shows "Deliver (mo, Append mo e) \<in> set xs"
proof -  
...
using assms proof (induct xs rule: rev_induct, clarsimp)
  ...
  proof (cases x, force)
    ...
    ultimately show ?thesis
      ...
      apply (simp)
      by (metis Un_iff added_ids_Deliver_Expunge_diff_collapse added_ids_Deliver_Expunge_same_collapse empty_iff expunge_id_valid list.set(1) list.set_intros(1) prefix_of_appendD set_ConsD set_append)
  qed
qed
...
lemma (in orset) Broadcast_Deliver_prefix_closed:
  assumes "xs @ [Broadcast (r, Rem ix e)] prefix of j"
    and "i \<in> ix"
  shows "Deliver (i, Add i e) \<in> set xs \<or> Deliver (i, Append i e) \<in> set xs \<or> (\<exists> mo . Deliver (i, Expunge e mo i) \<in> set xs)"
proof -  
  obtain y where "apply_operations xs = Some y"
    using assms broadcast_only_valid_msgs by blast
...
lemma (in orset) ids_are_unique:
  ...
  shows "i \<noteq> l \<and> i \<noteq> k \<and> l \<noteq> k"
  	using assms delivery_has_a_cause events_before_exist prefix_msg_in_history 
  	by (metis (no_types, lifting) fst_conv network.msg_id_unique network_axioms operation.distinct(11) operation.distinct(3) operation.distinct(5) prod.inject)
...
proof -
  ...
  moreover hence "Deliver (i, Add i e) \<in> set pre \<or> Deliver (i, Append i e) \<in> set pre \<or> (\<exists> mo . Deliver (i, Expunge e mo i) \<in> set pre)"
...
proof -
  obtain pre k where "pre@[Broadcast (r, Expunge e mo r)] prefix of k"
    using assms delivery_has_a_cause events_before_exist prefix_msg_in_history by blast
  moreover hence "Deliver (mo, Append mo e) \<in> set pre"
...
proof -
  obtain pre k where "pre@[Broadcast (ir, Rem is e2)] prefix of k"
   ...
   moreover hence "Deliver (i, Add i e2) \<in> set pre \<or> Deliver (i, Append i e2) \<in> set pre \<or> (\<exists> mo . Deliver (i, Expunge e2 mo i) \<in> set pre)"
  ...
  hence "Deliver (i, Expunge e2 mo i) \<in> set pre" using assms(2) assms(3) assms(1) ids_are_unique 
    by (smt calculation delivery_has_a_cause fst_conv network.msg_id_unique network_axioms operation.distinct(11) operation.distinct(5) operation.inject(4) prefix_elem_to_carriers prefix_msg_in_history prefix_of_appendD prod.inject)
...
proof -
  obtain pre k where "pre@[Broadcast (r, Expunge e2 mo r)] prefix of k"
   ...
   moreover hence "Deliver (mo, Append mo e2) \<in> set pre"
...
proof -
  { fix a b x y
    ...
    hence "interp_msg (a, b) \<rhd> interp_msg (x, y) = interp_msg (x, y) \<rhd> interp_msg (a, b)" sledgehammer
      apply(unfold interp_msg_def, case_tac "b"; case_tac "y"; simp add: add_add_commute rem_rem_commute append_append_commute add_append_commute add_expunge_commute expunge_expunge_commute hb.concurrent_def)
       ...
       by (metis append_expunge_commute append_id_valid assms concurrent_append_expunge_independent expunge_id_valid prefix_contains_msg)
