...
text\<open>In this final section show that concurrent updates commute and thus Strong Eventual Convergence is achieved.\<close>
...
corollary (in imap) concurrent_create_delete_independent:
  ...
  shows "i \<notin> is"
  using assms create_delete_ids_imply_messages_same concurrent_create_delete_independent_technical by fastforce
...
corollary (in imap) concurrent_append_delete_independent:
  assumes "\<not> hb (i, Append i e1) (ir, Delete is e2)" 
    ...
    and "(ir, Delete is e2) \<in> set (node_deliver_messages xs)"
  shows "i \<notin> is"
  using assms append_delete_ids_imply_messages_same concurrent_append_delete_independent_technical by fastforce
...
corollary (in imap) concurrent_expunge_delete_independent:
  assumes "\<not> hb (i, Expunge e1 mo i) (ir, Delete is e2)" 
    ...
    and "(ir, Delete is e2) \<in> set (node_deliver_messages xs)"
  shows "i \<notin> is"
  using assms expunge_delete_ids_imply_messages_same concurrent_expunge_delete_independent_technical by fastforce
...
corollary (in imap) concurrent_store_delete_independent:
  assumes "\<not> hb (i, Store e1 mo i) (ir, Delete is e2)" 
    ...
    and "(ir, Delete is e2) \<in> set (node_deliver_messages xs)"
  shows "i \<notin> is"
  using assms store_delete_ids_imply_messages_same concurrent_store_delete_independent_technical by fastforce
...
proof -
  { fix a b x y
    ...
    hence "interp_msg (a, b) \<rhd> interp_msg (x, y) = interp_msg (x, y) \<rhd> interp_msg (a, b)" 
      apply(unfold interp_msg_def, case_tac "b"; case_tac "y"; simp add: create_create_commute delete_delete_commute append_append_commute create_append_commute create_expunge_commute create_store_commute expunge_expunge_commute hb.concurrent_def)
      using assms prefix_contains_msg apply (metis (full_types) create_id_valid create_delete_commute concurrent_create_delete_independent)
      using assms prefix_contains_msg apply (metis (full_types) create_id_valid create_delete_commute concurrent_create_delete_independent)
      using assms prefix_contains_msg apply (metis append_id_valid append_delete_ids_imply_messages_same concurrent_append_delete_independent_technical delete_append_commute)
      using assms prefix_contains_msg apply (metis  concurrent_expunge_delete_independent expunge_id_valid imap.delete_expunge_commute imap_axioms)
      using assms prefix_contains_msg apply (metis assms concurrent_store_delete_independent delete_store_commute store_id_valid)
      using assms prefix_contains_msg apply (metis append_id_valid append_delete_ids_imply_messages_same concurrent_append_delete_independent_technical delete_append_commute)
      ...
      using assms prefix_contains_msg apply (metis concurrent_expunge_delete_independent expunge_id_valid delete_expunge_commute)
      ...
      using assms prefix_contains_msg apply (metis assms concurrent_store_delete_independent prefix_contains_msg delete_store_commute store_id_valid)
