...
proof -
  obtain pre k where P: "pre@[Broadcast (ir, Rem is e)] prefix of k"
  ...
  moreover hence "Deliver (i, Add i e) \<in> set pre \<or> Deliver (i, Append i e) \<in> set pre \<or> (\<exists> mo . Deliver (i, Expunge e mo i) \<in> set pre) \<or> (\<exists> mo . Deliver (i, Store e mo i) \<in> set pre)"
    using Broadcast_Deliver_prefix_closed assms(1) P by auto    	
  hence "Deliver (i, Add i e) \<in> set pre" using network.msg_id_unique network.delivery_has_a_cause using assms(2) assms(3) ids_are_unique P
  proof -
  	obtain aa :: 'a and aaa :: 'a where
  		f1: "(Deliver (i, Append i e) \<in> set pre \<or> (\<exists>a. Deliver (i, Expunge e a i) \<in> set pre) \<or> (\<exists>a. Deliver (i, Store e a i) \<in> set pre)) = (Deliver (i, Append i e) \<in> set pre \<or> Deliver (i, Expunge e aaa i) \<in> set pre \<or> Deliver (i, Store e aa i) \<in> set pre)"
  		by blast
  	...
  	obtain nn :: "'a \<times> ('a, 'b) operation \<Rightarrow> nat" where
		...
		then have f4: "Broadcast (i, Add i e) \<in> set (history (nn (i, Add i e)))"
		...
		then have f6: "Deliver (i, Append i e) \<in> set pre \<longrightarrow> nn (i, Append i e) = nn (i, Add i e) \<and> (i, Append i e) = (i, Add i e)"
			using f4 f3 f2 by (metis (no_types) calculation fst_conv)
		have f7: "Deliver (i, Expunge e aaa i) \<in> set pre \<longrightarrow> nn (i, Expunge e aaa i) = nn (i, Add i e) \<and> (i, Expunge e aaa i) = (i, Add i e)"
			using f5 f4 f3 f2 by (metis (no_types) calculation fst_conv)
		have "Deliver (i, Store e aa i) \<in> set pre \<longrightarrow> Deliver (i, Add i e) \<in> set pre"
			using f5 f4 f3 f2 by (metis (no_types) calculation fst_conv)
		then show ?thesis
			using f7 f6 f1 \<open>Deliver (i, Add i e) \<in> set pre \<or> Deliver (i, Append i e) \<in> set pre \<or> (\<exists>mo. Deliver (i, Expunge e mo i) \<in> set pre) \<or> (\<exists>mo. Deliver (i, Store e mo i) \<in> set pre)\<close> by blast
	qed  	
  ultimately show ?thesis
...
proof -
  ...
  proof -
  	...
  	then show ?thesis
  		using \<open>Deliver (i, Add i e) \<in> set pre \<or> Deliver (i, Append i e) \<in> set pre \<or> (\<exists>mo. Deliver (i, Expunge e mo i) \<in> set pre) \<or> (\<exists>mo. Deliver (i, Store e mo i) \<in> set pre)\<close> by fastforce
  qed 
    ultimately show ?thesis
...
proof -
  obtain pre k where P: "pre@[Broadcast (ir, Rem is e)] prefix of k"
    using assms delivery_has_a_cause events_before_exist prefix_msg_in_history by blast
  moreover hence A: "Deliver (i, Add i e) \<in> set pre \<or> Deliver (i, Append i e) \<in> set pre \<or> (\<exists> mo . Deliver (i, Expunge e mo i) \<in> set pre) \<or> (\<exists> mo . Deliver (i, Store e mo i) \<in> set pre)"
    using Broadcast_Deliver_prefix_closed assms(1) by auto
...
lemma (in imap) concurrent_store_expunge_independent_technical2:
    assumes "xs prefix of j"
    and "(i, Store e1 mo2 i) \<in> set (node_deliver_messages xs)" and "(r, Expunge e mo r) \<in> set (node_deliver_messages xs)"
  shows "mo2 \<noteq> r"
proof -
  obtain nn :: "'a \<times> ('a, 'b) operation \<Rightarrow> nat" where
...
lemma (in imap) concurrent_store_store_independent_technical:
    assumes "xs prefix of j"
    and "(i, Store e mo i) \<in> set (node_deliver_messages xs)" and "(r, Store e i r) \<in> set (node_deliver_messages xs)"
...
lemma (in imap) expunge_rem_ids_imply_messages_same:
  ...
  shows "e1 = e2"
proof -
  obtain pre k where P: "pre@[Broadcast (ir, Rem is e2)] prefix of k"
    using assms delivery_has_a_cause events_before_exist prefix_msg_in_history by blast  
   moreover hence A: "Deliver (i, Add i e2) \<in> set pre \<or> Deliver (i, Append i e2) \<in> set pre \<or> (\<exists> mo . Deliver (i, Expunge e2 mo i) \<in> set pre) \<or> (\<exists> mo . Deliver (i, Store e2 mo i) \<in> set pre)"
  ...
  proof -
  	...
  	proof -
  		...
  		then have "Deliver (i, Append i e2) \<notin> set (history k)"
  			using f5 f4 f3 f2 by (metis (no_types) added_files_Deliver_Append_same_collapse added_files_Deliver_Expunge_collapse added_ids_Deliver_Expunge_same_collapse empty_set fst_conv)
  		then obtain aa :: 'a and aaa :: 'a where
  					f7: "Deliver (i, Add i e2) \<in> set pre \<or> Deliver (i, Expunge e2 aaa i) \<in> set pre \<or> Deliver (i, Store e2 aa i) \<in> set pre"
  			using f1 A \<open>pre prefix of k\<close> by blast
  		have f8: "Deliver (i, Store e2 aa i) \<notin> set pre"
  			using f6 f5 f4 f3 f2 f1 by (metis \<open>pre prefix of k\<close> added_files_Deliver_Expunge_collapse added_files_Deliver_Store_same_collapse added_ids_Deliver_Expunge_same_collapse empty_set fst_conv)
  		have "added_ids [Deliver (i, Add i e2)] e1 \<noteq> [] \<or> (i, Add i e2) \<noteq> (i, Expunge e1 mo i)"
  			by fastforce
  		then have "e2 = e1"
  			using f8 f7 f6 f5 f4 f3 f1 by (metis \<open>pre prefix of k\<close> added_ids_Deliver_Add_diff_collapse added_ids_Deliver_Expunge_diff_collapse empty_set expunge_id_not_in_set fst_conv)
  		then show ?thesis
...
proof -
  ...
  then have f4: "Broadcast (i, Store e1 mo i) \<in> set (history (nn (i, Store e1 mo i)))"
    using assms(2) assms(3) prefix_msg_in_history by blast
  obtain aa :: 'a and aaa :: 'a where
    f5: "Deliver (i, Add i e2) \<in> set pre \<or> Deliver (i, Append i e2) \<in> set pre \<or> Deliver (i, Expunge e2 aaa i) \<in> set pre \<or> Deliver (i, Store e2 aa i) \<in> set pre"
    using A by blast
  ...
  then have f7: "Deliver (i, Expunge e2 aaa i) \<notin> set pre"
  	...
  	have f8: "Deliver (i, Add i e2) \<notin> set (history k)"
  		using f4 f3 f2 f1 by (metis (full_types) added_files_Deliver_Add_collapse added_files_Deliver_Store_same_collapse empty_set fst_conv list.set_intros(1) network_axioms)
...
proof -
  ...
  hence "Deliver (i, Add i e2) \<in> set pre" using assms ids_are_unique P
proof -
  	obtain aa :: 'a and aaa :: 'a where
  	...
  	obtain nn :: "'a \<times> ('a, 'b) operation \<Rightarrow> nat" where
  		f3: "\<forall>p n. Deliver p \<notin> set (history n) \<or> Broadcast p \<in> set (history (nn p))"
  		by (metis (no_types) delivery_has_a_cause)
		then have f4: "Broadcast (i, Add i e2) \<in> set (history (nn (i, Add i e2)))"
			using assms prefix_msg_in_history P A
		proof -
			obtain aab :: 'a and aac :: 'a where
    x1: "Deliver (i, Add i e2) \<in> set pre \<or> Deliver (i, Append i e2) \<in> set pre \<or> Deliver (i, Expunge e2 aac i) \<in> set pre \<or> Deliver (i, Store e2 aab i) \<in> set pre"
    using A by blast
  have x2: "\<forall>f fa p n pa na. \<not> network f fa \<or> Broadcast (p::'a \<times> ('a, 'b) operation) \<notin> set (f n) \<or> Broadcast pa \<notin> set (f na) \<or> (fa p::'a) \<noteq> fa pa \<or> n = na \<and> p = pa"
    by (meson network.msg_id_unique)
  have x3: "Broadcast (i, Add i e1) \<in> set (history (nn (i, Add i e1)))"
    using assms(2) assms(3) f3 prefix_msg_in_history by blast
  then have x4: "Deliver (i, Store e2 aab i) \<notin> set (history k)"
    using x2 by (metis (full_types) added_files_Deliver_Add_collapse added_files_Deliver_Store_same_collapse empty_iff empty_set f3 fst_conv list.set_intros(1) network_axioms)
  have x5: "Deliver (i, Append i e2) \<notin> set (history k)"
    using x3 f2 by (metis (full_types) added_files_Deliver_Add_collapse added_files_Deliver_Append_same_collapse empty_iff empty_set f3 fst_conv list.set_intros(1))
...
qed 
		have f5: "\<forall>es n e. \<not> es prefix of n \<or> e \<notin> set es \<or> e \<in> set (history n)"
			using prefix_elem_to_carriers by blast
		then have f6: "Deliver (i, Append i e2) \<in> set pre \<longrightarrow> nn (i, Append i e2) = nn (i, Add i e2) \<and> (i, Append i e2) = (i, Add i e2)"
			using f4 f3 f2	by (metis calculation fst_conv node_histories.prefix_of_appendD node_histories_axioms) 

		have f7: "Deliver (i, Expunge e2 aaa i) \<in> set pre \<longrightarrow> nn (i, Expunge e2 aaa i) = nn (i, Add i e2) \<and> (i, Expunge e2 aaa i) = (i, Add i e2)"
			using f5 f4 f3 f2 by (metis calculation fst_conv node_histories.prefix_of_appendD node_histories_axioms)
		have "Deliver (i, Store e2 aa i) \<in> set pre \<longrightarrow> Deliver (i, Add i e2) \<in> set pre"
			using f5 f4 f3 f2 by (metis calculation fst_conv node_histories.prefix_of_appendD node_histories_axioms)
		then show ?thesis
  ...
  hence "Deliver (i, Append i e2) \<in> set pre" using assms ids_are_unique 
		proof -
			...
			then have f3: "Broadcast (i, Append i e1) \<in> set (history (nn (i, Append i e1)))"
				using assms(2) assms(3) prefix_msg_in_history by blast
			obtain aa :: 'a and aaa :: 'a where
				f4: "(Deliver (i, Add i e2) \<in> set pre \<or> (\<exists>a. Deliver (i, Expunge e2 a i) \<in> set pre) \<or> (\<exists>a. Deliver (i, Store e2 a i) \<in> set pre)) = (Deliver (i, Add i e2) \<in> set pre \<or> Deliver (i, Expunge e2 aaa i) \<in> set pre \<or> Deliver (i, Store e2 aa i) \<in> set pre)"
				by blast
			...
			have f8: "Deliver (i, Add i e2) \<notin> set pre"
				using f6 f5 f3 f2 f1 by (metis (no_types) added_files_Deliver_Add_collapse added_files_Deliver_Append_same_collapse empty_set fst_conv list.set_intros(1) network_axioms)
...
proof -
  ...
  hence "Deliver (i, Append i e2) \<in> set pre" using assms ids_are_unique
  proof -
  	have f1: "Store e2 v0_0 mo \<noteq> Append i e1"
    by blast
  have f2: "Deliver (i, Append i e1) \<in> set (history j)"
    using \<open>(i, Append i e1) \<in> set (node_deliver_messages xs)\<close> \<open>xs prefix of j\<close> prefix_msg_in_history by blast
  obtain aa :: 'a where
    f3: "Deliver (mo, Append mo e2) \<in> set pre \<or> Deliver (mo, Store e2 aa mo) \<in> set pre"
    using \<open>Deliver (mo, Append mo e2) \<in> set pre \<or> (\<exists>mo2. Deliver (mo, Store e2 mo2 mo) \<in> set pre)\<close> by blast
    	have "pre prefix of k"
    		using calculation by blast
    	then have "Deliver (mo, Store e2 aa mo) \<notin> set pre"
    		using f2 f1  Pair_inject assms delivery_has_a_cause fst_conv msg_id_unique prefix_elem_to_carriers
    		by (metis operation.distinct(17)) 
    	then have "Deliver (mo, Append mo e2) \<in> set pre"
    		using f3 by meson
    	then show ?thesis
    		by (simp add: \<open>i = mo\<close>)
    qed       
  	moreover have "Deliver (i, Append i e1) \<in> set (history j)"
    using assms(2) assms(3) prefix_msg_in_history by blast
  ultimately show ?thesis
...
proof -
  obtain pre k where P: "pre@[Broadcast (r, Store e2 mo r)] prefix of k"
   ...
   moreover hence A: "Deliver (mo, Append mo e2) \<in> set pre \<or> (\<exists> mo2 . Deliver (mo, Store e2 mo2 mo) \<in> set pre)"
    using Broadcast_Store_Deliver_prefix_closed assms(1)
    by (meson imap.Broadcast_Deliver_prefix_closed imap_axioms)
    	
  have "Deliver (i, Append i e2) \<in> set pre" using assms ids_are_unique P A
  proof -
  	have f1: "Deliver (i, Append i e1) \<in> set (history j)"
  		using assms(2) assms(3) prefix_msg_in_history by blast
  	obtain aa :: 'a where
  		f2: "Deliver (mo, Append mo e2) \<in> set pre \<or> Deliver (mo, Store e2 aa mo) \<in> set pre"
  		using A by blast
  	have f3: "pre prefix of k"
  		using calculation by blast
  	have f4: "fst (mo, Store e2 aa mo) = fst (i, Append i e1)"
  		using assms(1) by auto
  	have "v2_1 (mo, Store e2 aa mo) \<noteq> (v2_1 (i, Append i e1)::nat) \<or> (mo, Store e2 aa mo) \<noteq> (i, Append i e1)"
  		by force
  	then have "Deliver (mo, Store e2 aa mo) \<notin> set (history k)"
  		using f4 f1 by (metis (no_types) delivery_has_a_cause network.msg_id_unique network_axioms)
  	then show ?thesis
  		using f3 f2 assms(1) prefix_elem_to_carriers by blast
  qed      
  	moreover have "Deliver (i, Append i e1) \<in> set (history j)"
    using assms(2) assms(3) prefix_msg_in_history by blast
  ultimately show ?thesis
...
proof -
  obtain pre k where P: "pre@[Broadcast (r, Expunge e2 i r)] prefix of k"
   ...
   moreover hence A: "Deliver (i, Append i e2) \<in> set pre \<or> (\<exists> mo2 . Deliver (i, Store e2 mo2 i) \<in> set pre)"
    using Broadcast_Expunge_Deliver_prefix_closed assms(1) by blast
    	
  have B: "Deliver (i, Store e2 mo i) \<in> set pre" using assms ids_are_unique A P
  proof -
  	obtain nn :: "'a \<times> ('a, 'b) operation \<Rightarrow> nat" where
  		f1: "\<And>p n. Deliver p \<notin> set (history n) \<or> Broadcast p \<in> set (history (nn p))"
  		by (metis (no_types) delivery_has_a_cause)
  	then have "Broadcast (i, Store e2 mo i) \<in> set (history (nn (i, Store e2 mo i)))"
  		using assms(1) assms(2) prefix_msg_in_history
proof -
  have x1: "Broadcast (i, Store e1 mo i) \<in> set (history (nn (i, Store e1 mo i)))"
    using assms(1) assms(2) f1 prefix_msg_in_history by blast
  have x2: "\<forall>b a aa ba ab ac. (Store (b::'b) (a::'a) aa = Store ba ab ac) = (b = ba \<and> a = ab \<and> aa = ac)"
    by blast
  have x3: "\<forall>a z aa za. (a::'a, z::('a, 'b) operation) \<noteq> (aa, za) \<or> a = aa \<and> z = za"
    by blast
  have x4: "(\<forall>a z aa za. (a::'a, z::('a, 'b) operation) \<noteq> (aa, za) \<or> a = aa \<and> z = za) = (\<forall>a z aa za. (a::'a, z::('a, 'b) operation) \<noteq> (aa, za) \<or> a = aa \<and> z = za)"
    by meson
    	have x5: "\<forall>p n pa na. Broadcast p \<notin> set (history n) \<or> Broadcast pa \<notin> set (history na) \<or> fst p \<noteq> fst pa \<or> n = na \<and> p = pa"
    		using msg_id_unique by blast
    	then obtain aa :: 'a where
    		"Deliver (i, Store e2 aa i) \<in> set (history k)"
    using x3 x1 by (metis A calculation f1 fst_conv node_histories.prefix_of_appendD node_histories_axioms operation.distinct(17) prefix_elem_to_carriers)
  then have "e2 = e1 \<and> aa = mo"
    using x5 x4 x3 x2 x1 by (metis (no_types) f1 fst_conv)
  then show ?thesis
    using x1 by force
qed  
  	then show ?thesis
  		using f1 by (metis (no_types) A calculation fst_conv msg_id_unique prefix_elem_to_carriers prefix_of_appendD)
  qed 
  	moreover have "Deliver (i, Store e1 mo i) \<in> set (history j)"
    using assms(1) assms(2) prefix_msg_in_history by auto
  ultimately show ?thesis using assms
  	by (metis delivery_has_a_cause fst_conv msg_id_unique operation.inject(5) prefix_elem_to_carriers prefix_of_appendD prod.inject) 
qed 
	
lemma (in imap) store_store_ids_imply_messages_same:
    assumes "xs prefix of j"
    and "(i, Store e1 mo i) \<in> set (node_deliver_messages xs)" and "(r, Store e2 i r) \<in> set (node_deliver_messages xs)"
  shows "e1 = e2"
proof -
  obtain pre k where P: "pre@[Broadcast (r, Store e2 i r)] prefix of k"
   ...
   moreover hence A: "Deliver (i, Append i e2) \<in> set pre \<or> (\<exists> mo2 . Deliver (i, Store e2 mo2 i) \<in> set pre)"
   	 using Broadcast_Store_Deliver_prefix_closed assms(1) by blast
   	 	 ...
   	 	 proof -
   	 	 	 obtain aa :: 'a where
					f1: "Deliver (i, Append i e2) \<in> set pre \<or> Deliver (i, Store e2 aa i) \<in> set pre"
					using A by blast
				have f2: "\<forall>e. e \<notin> set pre \<or> e \<in> set (history k)"
					by (metis (no_types) calculation node_histories.prefix_of_appendD node_histories_axioms prefix_to_carriers subset_iff)
				have f3: "\<forall>f fa p n pa na. \<not> network f fa \<or> Broadcast (p::'a \<times> ('a, 'b) operation) \<notin> set (f n) \<or> Broadcast pa \<notin> set (f na) \<or> (fa p::'a) \<noteq> fa pa \<or> n = na \<and> p = pa"
					by (meson network.msg_id_unique)
				obtain nn :: "'a \<times> ('a, 'b) operation \<Rightarrow> nat" where
					f4: "\<forall>p n. Deliver p \<notin> set (history n) \<or> Broadcast p \<in> set (history (nn p))"
					by (metis delivery_has_a_cause)
				then have f5: "Broadcast (i, Store e1 mo i) \<in> set (history (nn (i, Store e1 mo i)))"
					using assms(1) assms(2) prefix_msg_in_history by blast
				then have f6: "Deliver (i, Append i e2) \<notin> set pre"
					using f4 f3 f2 by (metis (no_types) Pair_inject fst_conv network_axioms operation.distinct(17))
				then have "Broadcast (i, Store e2 aa i) \<in> set (history (nn (i, Store e2 aa i)))"
					using f4 f2 f1 by blast
				then have "nn (i, Store e2 aa i) = nn (i, Store e1 mo i) \<and> (i, Store e2 aa i) = (i, Store e1 mo i)"
					using f5 f3 by (metis fst_conv network_axioms)
				then show ?thesis
					using f6 f1 by force
				qed
  	moreover have "Deliver (i, Store e1 mo i) \<in> set (history j)"
    using assms(1) assms(2) prefix_msg_in_history by auto
  ultimately show ?thesis using assms
