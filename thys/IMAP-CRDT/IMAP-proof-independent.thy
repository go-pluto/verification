...
text\<open>In this section we show that two concurrent operations that reference to the same tag must be identical.\<close>
...
using assms proof (induct xs rule: rev_induct, clarsimp)
  ...
  proof (cases x, force)
    ...
    ultimately show ?thesis
      ...
      apply (simp,metis added_ids_Deliver_Create_diff_collapse added_ids_Deliver_Create_same_collapse
...
proof -
  ...
  hence "Deliver (i, Create i e) \<in> set pre" using network.msg_id_unique network.delivery_has_a_cause using assms(2) assms(3) ids_are_unique P
  proof -
  ...
  proof -
    ...
    proof -
      ...
      have "added_ids [Deliver (i, Create i e2)] e1 \<noteq> [] \<or> (i, Create i e2) \<noteq> (i, Expunge e1 mo i)"
        by fastforce
      then have "e2 = e1"
        using f8 f7 f6 f5 f4 f3 f1 by (metis \<open>pre prefix of k\<close> added_ids_Deliver_Create_diff_collapse added_ids_Deliver_Expunge_diff_collapse empty_set expunge_id_not_in_set fst_conv)
      then show ?thesis
...
proof -
  ...
  hence "Deliver (i, Append i e2) \<in> set pre" using assms ids_are_unique
  proof -
    have f1: "Store e2 X mo \<noteq> Append i e1"
    by blast
